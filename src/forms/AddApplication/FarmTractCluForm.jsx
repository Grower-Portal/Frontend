...
function FarmTractCluForm({ rows, setRows, farms, setFarms, onPrevious, onNext }) {
  ...
  const fetchCluCalculatedAcreage = useCallback(async (farmNumber, tractNumber, cluNumber, index, cluIndex) => {
    try {
      ...
      const response = await fetch(`https://grower-portal-412701.uc.r.appspot.com/api/clula/findCluCalculatedAcreage?farmNumber=${farmNumber}&tractNumber=${tractNumber}&cluNumber=${cluNumber}`, {
        method: 'GET',
        headers: {
          'accept': '*/*',
        },
  ...
  const fetchFsaPhysicalLocation = useCallback(async (farmNumber, tractNumber, cluNumber, index, cluIndex) => {
    ...
    setRows(newRows);
      const response = await fetch(`https://grower-portal-412701.uc.r.appspot.com/api/clula/findFsaPhysicalLocation?farmNumber=${farmNumber}&tractNumber=${tractNumber}&cluNumber=${cluNumber}`);
      if (response.ok) {
        const contentType = response.headers.get('content-type');
        if (contentType && contentType.includes('application/json')) {
          // If the response is JSON, parse it as JSON
